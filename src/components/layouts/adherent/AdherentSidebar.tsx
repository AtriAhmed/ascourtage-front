...
import { people, helpCircle, person, logOut, apps, calculator } from 'ionicons/icons';
...
const AdherentSidebar: React.FC<AdherentSidebarProps> = ({ isExpanded, setIsExpanded }) => {

    const { user, userLoading, setUser } = useAuthContext();
    function logout() {
            ...
            <IonAccordionGroup className='w-full h-full relative z-10 sidebar' onClick={() => { if (!isExpanded) setIsExpanded(true) }}>
                <div className={`flex flex-col ${isExpanded ? "gap-2" : "gap-14"} p-4 ${isExpanded ? "" : "pt-10"}`}>
                    ...
                    <div className={`${BUTTON_STYLE}`}>
                        ...
                        {isExpanded && <IonItem className={`${ITEM_STYLE}`} onClick={() => navigateAndCloseMenu("/adherent/dashboard")}>
                    ...
                    >
                        ...
                        {isExpanded && <IonAccordion value="first" className={`${ACC_STYLE}`}>
                            <IonItem slot="header" >
                                <IonLabel className={`${LABEL_STYLE}`}>Mes Informations</IonLabel>
                            </IonItem>
                            <div className="p-4 pl-8" slot="content" onClick={() => { navigateAndCloseMenu("/adherent/details") }}>
                                Adhérent
                            </div>
                            <div className="p-4 pl-8" slot="content" onClick={() => { navigateAndCloseMenu(`/prestataires/by-adherent/${user?.code_matricule_id}`); }}>
                                Prestataires
                            </div>
                        </IonAccordion>}
                    </div>
                    ...
                    <div className={`${BUTTON_STYLE}`}>
                        ...
                        {isExpanded && <IonItem className={`${ITEM_STYLE}`} onClick={() => navigateAndCloseMenu("/adherent/decomptes")}>
                            <IonLabel className={`${LABEL_STYLE}`}>Etat des décomptes</IonLabel>
                        </IonItem>}
                    </div>
